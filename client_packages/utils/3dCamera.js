global.CameraRotator = class {
    ...
    onMouseMove(dX, dY) {
        ...
        if (
            relativeHeading > this.xBound[0] &&
            relativeHeading < this.xBound[1]
        ) {
            relativeHeading =
                Math.abs(this.xBound[0] - relativeHeading) >
                Math.abs(this.xBound[1] - relativeHeading)
                    ? this.xBound[1]
                    : this.xBound[0];
        }

        this.heading = this.normilizeHeading(
            -relativeHeading + this.baseHeading
        );
...
mp.events.add("render", () => {
    if (
        ...
        !cameraRotator.isActive ||
        cameraRotator.isPause
    ) {
        return;
    }
...
global.createCam = function () {
    ...
    message += `\nRelative Heading: ${cameraRotator
        .getRelativeHeading()
        .toFixed(2)}`;
