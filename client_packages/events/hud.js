...
mp.events.add("render", () => {
    ...
    let plocation = mp.game.zone.getNameOfZone(
        player.position.x,
        player.position.y,
        player.position.z
    );
    let location = mp.game.ui.getLabelText(plocation);
    let zone_hash = mp.game.pathfind.getStreetNameAtCoord(
        player.position.x,
        player.position.y,
        player.position.z,
        0,
    ...
    browser.call(
        "HUD_updateLocation::CEF",
        getMinimapTopRight().y + 130,
        getMinimapTopRight().x + 15,
        location,
        zone
    );
    ...
    browser.execute(
        `HUD.server.login = '${player.getVariable(
            "login"
...
mp.events.add("Hud_updateMoney::CLIENT", (money, bank) => {
    ...
    if (bank == undefined)
        return browser.execute(`HUD.money = '${money}'; HUD.bank = '0';`);
